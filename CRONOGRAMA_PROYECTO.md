...
*   **Día 5: Gastos Generales**
    *   Análisis y carga de la hoja 'Gastos' (filtrando compras de mercadería).
    *   Normalización inicial de `proveedores` de servicios.
    *   Carga de tabla `gastos` (Luz, Agua, Alquiler, Publicidad, etc.).

## Fase 3: Compras, Inventario y Hosting (Semana 3)

*   **Día 1: Análisis de Compras y Costos (Nuevo)**
    *   Revisar hojas 'Facturas', 'Evolución costos' y 'Gastos' (parte de mercadería).
    *   Diferenciar entre "Gasto Operativo" y "Compra de Inventario".
    *   Diseñar lógica para vincular Compras con Productos existentes.

*   **Día 2: Desarrollo ETL - Compras e Inventario**
    *   Escribir script para cargar tabla `compras` (Cabecera).
    *   Escribir script para cargar `detalle_compras` (Items), asegurando el costo unitario.
    *   Validar integridad referencial con `proveedores` y `productos`.

*   **Día 3: Integración y Pruebas Generales**
    *   Ejecutar pipeline completo: Clientes -> Productos -> Compras -> Ventas -> Gastos.
    *   Validación de datos (QA): Verificar que los costos en compras coincidan con el histórico.
    *   Resolver errores de claves foráneas.

*   **Día 4: Investigación y Selección de Hosting**
    ...
    *   Seleccionar proveedor y crear cuenta inicial.

*   **Día 5: Configuración y Despliegue en Nube**
    *   Provisionar la base de datos remota.
    *   Ejecutar `schema.sql` en la nube usando DBeaver o CLI.
    *   Probar conexión remota desde el script Python local.
